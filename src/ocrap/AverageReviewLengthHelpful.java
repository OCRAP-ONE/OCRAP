...
public class AverageReviewLengthHelpful {
	
	public static void main(String[] args) throws Exception {
		
		SparkConf sparkConf = new SparkConf().setAppName("OCRAP_AverageReviewLengthHelpful").setMaster("local");
		...
		// transformation: form key-value-pairs such that review id (key) and review description (value) take only reviews into account with helpfulness ratio of 1.0
			...
			public Tuple2<String, String> call(String review) {
				...
				for (String line : lines) {
					...
					if (elements[0].equals("product/productId:")) {
						...
						key = tmp + elements[1];
					} if (elements[0].equals("review/helpfulness:")) {
						String[] helpfulnessRatio = elements[1].split("/");
						String counter = helpfulnessRatio[0];
						String denominator = helpfulnessRatio[1];
						if (counter.equals(denominator) && !counter.equals("0")) {
							for (String line2 : lines) {
								String[] elements2 = line2.split(" ", 2);
								if (elements2[0].equals("review/text:")) {
									result = new Tuple2<String, String>(key, elements2[1]);
								}
							}
						}
					}
